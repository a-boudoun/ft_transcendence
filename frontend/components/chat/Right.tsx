...
const Right = () => {
  ...
  return (
    <div className={`${isMid === false ? 'flex flex-col w-1/2': 'hidden'} lg:w-3/12 bg-white bg-opacity-20 ackdrop-blur-lg  drop-shadow-lg rounded-xl`}>
      <button onClick={()=> dispatch(setisMid(true))}>
      <Image
        className="h-[30px] w-[30px]  rounded-full  m-2"
        src={'/img/cancel.svg'}
        width={1000}
        height={1000}
      />
      ...
      <div className="h-1/3 w-[96%] m-2 rounded-xl">
        ...
        <div>
          ...
          {data.memberships?.map((member: any) => (
            <Items key={member.id} data={member} />
          ))}
        </div>
      </div>
