...
function LoadingPlayer({setClicked, setGame}: prop){
	...
	function RightPlayer({ clicked, setClicked, setGame } : prop){
		...
		return (
			<div className = 'flex flex-col iterms-center justify-center  w-[150px] h-[150px]'>
				{clicked === 1 || clicked === 2 ? <LoadingPlayer setClicked={setClicked} setGame={setGame}/>  :   <Image src="/game/unknown.svg" width={200} height={200} alt="unkown" className="w-full h-full rounded-full"/>}
			</div>
		)
	}
